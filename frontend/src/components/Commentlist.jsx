...
function Commentlist() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/patelcomment")
      .then((response) => {
        console.log(response.data);
        const commentArray = response.data.map(
      ...
      <div className="commentlist-second-outer container">
        ...
        <div className="row">
          <div className="comment-card col-12 col-sm-8 col-md-8 col-lg-8 mb-8">
            <CommentSegmentSlide comment="Lorem ipsum dolor, sit amet consectetur adipisicing elit. Omnis soluta excepturi explicabo eius nam, quas aliquid eveniet provident quod ad." />
          </div>
          ...
          {comments.map((comment, index) => (
            ...
            >
              <CommentSegmentSlide comment={comment} />
            </div>
          ))}
        </div>
      </div>
